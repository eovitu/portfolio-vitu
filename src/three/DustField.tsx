...
const neutralDustColor = new THREE.Color(colors.dust);
...
export function DustField({ pointerRef, idle }: Props) {
  ...
  useFrame((state) => {
    ...
    material.opacity =
      Math.min(1, target) * Math.min(1, t / 2.6) * Math.max(0, sceneSignals.particles);
    const projectTheme = sceneSignals.projectTheme;
    const theme = projectTheme ? PROJECT_THEMES[projectTheme] : null;
    const targetColor = projectTheme ? themeDustColors[projectTheme] : neutralDustColor;
    material.color.lerp(targetColor, 0.045);
    spread.current += ((theme?.particleSpread ?? 1) - spread.current) * 0.045;
    node.scale.set(spread.current, 0.92 + spread.current * 0.08, spread.current);
    ...
    uniforms.uLens.value =
      (0.5 + Math.max(heroSignal.energy, sceneSignals.energy) * 0.9) *
      Math.min(1, stage().presence);
  });
